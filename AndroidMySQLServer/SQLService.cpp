...
void SQLService::ServiceRequest(std::vector<char>& data)
{
	...
	case GET_ITEM_BANK_ITEMS:
		try {
			...
			mySqlSmartPointer count(_mysql.executeQuery("SELECT COUNT(*) FROM items"));
			count()->next();
			unsigned short numOfRecords = count.get()->getInt(1);

			_DESCRIPTIVE_CONSOLE_OUT(to_string((int)numOfRecords));

			mySqlSmartPointer res(_mysql.executeQuery("SELECT * FROM items"));
	...
	case GET_LISTS:
		try {
			...
			mySqlSmartPointer count(_mysql.executeQuery("SELECT COUNT(*) FROM lists"));
			count()->next();
			unsigned short numOfRecords = count()->getInt(1);

			_DESCRIPTIVE_CONSOLE_OUT(to_string((int)numOfRecords));

			mySqlSmartPointer res(_mysql.executeQuery("SELECT * FROM lists"));
			std::vector<char> sendData;
	...
	case GET_LIST_ITEMS:
		try {
			...
			mySqlSmartPointer count(_mysql.executeQuery("SELECT COUNT(*) FROM listitems"));
			count()->next();
			unsigned short numOfRecords = count.get()->getInt(1);

			_DESCRIPTIVE_CONSOLE_OUT(to_string((int)numOfRecords));

			mySqlSmartPointer res(_mysql.executeQuery("SELECT * FROM listitems"));
			std::vector<char> sendData;
			sendData.push_back(GIVE_LIST_ITEMS);
			sendData.push_back((char)(numOfRecords & 0xff));
			sendData.push_back((char)((numOfRecords >> 8) & 0xff));
			auto listItemItemSize = getListItemItemSize();
			sendData.push_back((char)(listItemItemSize & 0xff));
			sendData.push_back((char)((listItemItemSize >> 8) & 0xff));

			while (res()->next()) {
				listItemItem lii;
				lii.id = res()->getInt("id");
				lii.quantity = res()->getInt("quantity");
				lii.itemId = res()->getInt("itemID");
				lii.listId = res()->getInt("listID");
				lii.checked = res()->getBoolean("CHECKED");
				stuff(&lii, sendData);
			}
...
void SQLService::stuff(listItemItem * lii, std::vector<char>& data)
{
	copy((char*)&lii->id, data, sizeof(lii->id));
	copy((char*)&lii->quantity, data, sizeof(lii->quantity));
	copy((char*)&lii->itemId, data, sizeof(lii->itemId));
	copy((char*)&lii->listId, data, sizeof(lii->listId));
	copy((char*)&lii->checked, data, sizeof(lii->checked));
}
...
unsigned short SQLService::getListItemItemSize()
{
	return (unsigned short)(sizeof(listItemItem::id) + sizeof(listItemItem::quantity) + sizeof(listItemItem::itemId) + sizeof(listItemItem::listId)
		+ sizeof(listItemItem::checked));
}
